...
select with cursor and wait few seconds to highlight volume labels, then click OK to proceed")
	repeat with the_Item in your_selected_device_id
		...
		if  tName contains "APFS" and (tName does not contain "VM" and tName does not contain "- D" and tName does not contain "Update" and tName does not contain "Preboot" and tName does not contain "Recovery" and tName does not contain "Container" and tName does not contain "Snapshot") then
			set Check to true
		end if
